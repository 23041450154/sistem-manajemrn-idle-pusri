...
export default function ManajemenInspeksi() {
  ...
  const getApprovalBadge = (status: ApprovalState) => {
    ...
    return <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${styles[status]}`}>{labels[status]}</span>;
  };

  const getActionButton = (asset: Asset) => {
    if (asset.statusPersetujuan === "PENDING") {
      return (
        <button onClick={() => openModal(asset)} className="text-[#0A356A] hover:text-[#0556B3] font-semibold text-[13px] flex items-center gap-1.5 transition-colors">
          <Check className="w-3.5 h-3.5" /> Validasi
        </button>
      );
    }
    if (asset.statusPersetujuan === "NEED_REVISION") {
      return (
        <button onClick={() => openModal(asset)} className="text-orange-600 hover:text-orange-700 font-semibold text-[13px] flex items-center gap-1.5 transition-colors">
    ...
    if (asset.statusPersetujuan === "IN_REVIEW" && asset.statusAset === "VALIDATED") {
      return (
        <button onClick={() => openModal(asset)} className="text-gray-600 hover:text-gray-900 font-semibold text-[13px] flex items-center gap-1.5 transition-colors">
          <Edit className="w-3.5 h-3.5" /> Ubah
        </button>
      );
    }
    
    // Default (Approved / IDLE / Rejected Final)
    return (
      <button onClick={() => openModal(asset)} className="text-gray-600 hover:text-gray-900 font-semibold text-[13px] flex items-center gap-1.5 transition-colors">
                ...
                filteredAssets.map((asset) => (
                  <tr key={asset.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-5 py-3">
                      <div className="text-[13px] font-medium text-gray-900">{asset.namaAlat}</div>
                      <div className="text-[11px] text-gray-500 mt-0.5">{asset.jenisAlat}</div>
                    </td>
                    ...
                    <td className="px-5 py-3 whitespace-nowrap text-[13px] text-gray-600">
                      {new Date(asset.tanggalRegistrasi).toLocaleDateString("id-ID", {
