...
export default function ManajemenInspeksi() {
	...
	const [sortConfig, setSortConfig] = useState<{
		key: keyof Asset;
		direction: "asc" | "desc";
	} | null>({ key: "tanggalRegistrasi", direction: "desc" });
					...
					assets.map((a) =>
						a.id === selectedAsset.id
							? {
									...
									statusPersetujuan: isUtilizable
										? "PENDING_REVIEW"
										: "REJECTED",
	...
	const getStatusAsetBadge = (status: AssetState | string) => {
		...
		return (
			<span
				className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${styles[status] || styles["READY TO USE"]}`}
			>
				{displayStatus}
			</span>
		);
	...
	const getApprovalBadge = (status: ApprovalState) => {
		...
		return (
			<span
				className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${styles[status]}`}
			>
				{labels[status]}
			</span>
		);
			...
			<div className="flex flex-wrap items-center gap-1 justify-center w-full max-w-[100px] mx-auto">
				...
				{asset.statusAset === "VALIDATED" &&
					asset.statusPersetujuan === "PENDING_REVIEW" && (
						<button
							title="Ubah Validasi"
							onClick={() => openModal(asset, "VALIDASI")}
							className="text-orange-500 hover:text-orange-700 hover:bg-orange-50 p-0.5 rounded transition-colors flex flex-col items-center"
						>
				...
				{asset.statusPersetujuan === "NEED_REVISION" && (
					<button
						...
						onClick={() => openModal(asset, "VALIDASI")}
						className="text-purple-600 hover:text-purple-800 hover:bg-purple-50 p-0.5 rounded transition-colors flex flex-col items-center"
					>
						<Edit className="w-3 h-3 mb-0.5" />
		...
		<div className="max-w-7xl mx-auto pt-2 pb-8">
			...
			<div className="mb-4">
				...
				<p className="text-[13px] text-gray-500 mt-1">
					Daftar peralatan idle yang membutuhkan verifikasi teknis sebelum
					di-utilisasi.
				</p>
			</div>
				...
				<div className="mb-4 flex items-center justify-between bg-blue-50 border border-blue-100 rounded-lg px-4 py-2.5 animate-in fade-in slide-in-from-top-2">
					<div className="flex items-center gap-3">
						...
						<span className="text-[13px] text-blue-800 font-medium">
							...
							membutuhkan tindakan Validasi atau Revisi dari Anda.
						</span>
					</div>
					<button
			...
			<div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
									...
									>
										<td className="px-1.5 py-1 whitespace-nowrap text-[10px] font-bold text-[#0A356A] relative">
											{(asset.statusPersetujuan === "NONE" ||
												...
												asset.statusPersetujuan === "NEED_REVISION") && (
												<span
													className="absolute left-0 top-1/2 -translate-y-1/2 flex h-1.5 w-1.5"
													title={
														asset.statusPersetujuan === "NEED_REVISION"
															? "Perlu Revisi"
															: "Perlu Validasi"
												...
												>
													<span
														className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${asset.statusPersetujuan === "NONE" || asset.statusPersetujuan === "PENDING_REVIEW" ? "bg-red-400" : "bg-orange-400"}`}
													></span>
													<span
														className={`relative inline-flex rounded-full h-1.5 w-1.5 ${asset.statusPersetujuan === "NONE" || asset.statusPersetujuan === "PENDING_REVIEW" ? "bg-red-500" : "bg-orange-500"}`}
													></span>
												</span>
											)}
										...
										<td className="px-1.5 py-1 text-[10px]">
											{asset.statusPersetujuan === "NONE" ||
											asset.statusPersetujuan === "PENDING_REVIEW" ? (
												<span className="bg-[#DCFCE7] text-[#16A34A] px-1.5 py-0.5 rounded-full flex items-center gap-1 w-fit font-semibold text-[8px]">
													<div className="w-1.5 h-1.5 bg-[#16A34A] rounded-full"></div>{" "}
													1 Hari
												</span>
											) : (
												<span className="text-gray-400 font-medium text-[9px]">
													Selesai
												</span>
											)}
										</td>
				...
				<div className="px-5 py-3 border-t border-gray-200 bg-white flex justify-between items-center">
					<span className="text-[11px] font-medium text-gray-500">
						Menampilkan {paginatedAssets.length} dari {filteredAssets.length}{" "}
						data
					</span>
					{totalPages > 1 && (
								...
								<h2 className="text-base font-bold text-gray-900">
									{selectedAsset.statusPersetujuan === "NEED_REVISION"
										...
										: selectedAsset.statusPersetujuan === "PENDING_REVIEW"
											? "Ubah Validasi Inspeksi"
											: "Validasi Inspeksi Teknik"}
								</h2>
						...
						<div className="flex-1 overflow-y-auto px-6 py-4 bg-gray-50/50">
							...
							{selectedAsset.statusPersetujuan === "NEED_REVISION" && (
								<div className="bg-orange-50 border-l-4 border-orange-500 p-3 mb-4 rounded-r-lg flex gap-3 shadow-sm">
									...
									<div>
										<h4 className="text-[12px] font-bold text-orange-800">
											Menunggu Revisi Anda
										</h4>
										<p className="text-[11px] text-orange-700 mt-0.5">
											Manager meminta revisi: &quot;Mohon lampirkan bukti foto
											tambahan dan lengkapi detail kondisi pada catatan
											pemeriksaan.&quot;
										</p>
									</div>
								</div>
								...
								<div className="grid grid-cols-12 gap-3 mb-3">
									...
									<div className="col-span-3">
										...
										<input
											...
											disabled={isReadOnly}
											className={`w-full bg-white border rounded-md px-3 py-1.5 text-[13px] outline-none disabled:bg-gray-50 ${showValidationErrors && !tglPemeriksaan ? "border-red-400 focus:border-red-500" : "border-gray-300 focus:border-[#0A356A]"}`}
										/>
