...
export default function UnitKerjaDashboard() {
  ...
  const [reuseRequests, setReuseRequests] = React.useState<ApiRow[]>([]);
  ...
  if (pieData.length === 0)
    pieData.push({ name: "Belum Ada Data", value: 1, color: "#E5E7EB" });

  // Group by plant
  const plants = data.reduce((acc: Record<string, number>, curr: ApiRow) => {
    const plant =
      (typeof curr.plant === "object" ? curr.plant?.name : curr.plant) ||
      curr.plant_description ||
      "Lainnya";
            ...
            {pieData.map((item, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-gray-600">
                  {item.name} (
                  {totalAset > 0
                    ? Math.round((item.value / totalAset) * 100)
                    : 0}
                  %)
                </span>
              </div>
