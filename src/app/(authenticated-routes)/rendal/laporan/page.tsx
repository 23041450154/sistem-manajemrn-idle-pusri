...
import { getEquipments, getApprovals, getInspections, getDisposals } from "@/action/api";
...
interface AuditLogEntry {
  ...
  actorRole: string;
  action: string;
  actionType: "CREATE" | "UPDATE" | "DELETE" | "APPROVE" | "REJECT" | "INSPECT" | "UPLOAD" | "REVIEW";
  module: string;
  entityCode: string;
  entityName: string;
  description: string;
...
function buildAuditLogs(
  ...
  disposals.forEach((disp: any) => {
    logs.push({
      ...
      actorRole: "Rendal Pemeliharaan",
      action: disp.status === "DISPOSED" ? "Disposal Disetujui" : "Pengajuan Disposal",
...
export default function AuditTrailPage() {
  ...
  useEffect(() => {
    async function fetchAuditData() {
      ...
      try {
        ...
        const auditLogs = buildAuditLogs(eq || [], apps || [], ins || [], disps || []);
        setLogs(auditLogs);
      } catch (err) {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      ...
      const matchAction = actionFilter === "Semua" || log.actionType === actionFilter;
  ...
  const handleExport = () => {
    const headers = ["Timestamp", "Aktivitas", "Tipe", "Modul", "Kode Entitas", "Nama Entitas", "Aktor", "Role", "Deskripsi", "Nilai Lama", "Nilai Baru"];
    ...
    <div className="max-w-7xl mx-auto pt-6 pb-8 px-6 font-sans">
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-gray-900 tracking-tight">Laporan Log Audit Trail</h1>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isLoading}
              className="flex items-center justify-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-[13px] font-semibold text-gray-700 hover:bg-gray-50 hover:text-[#0A356A] transition-colors shadow-sm disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#0A356A] hover:bg-[#0556B3] text-white rounded-lg text-[13px] font-semibold transition-colors shadow-sm"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Total Log</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Pembuatan</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Pembaruan</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Inspeksi</span>
          </div>
      ...
      <div className="bg-white p-4 border border-gray-200 rounded-t-xl shadow-sm flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 text-[13px] bg-white border border-gray-300 rounded-lg focus:border-[#0A356A] focus:ring-1 focus:ring-[#0A356A] transition-all outline-none placeholder:text-gray-400"
            />
          </div>
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button
              onClick={() => setShowFilters(!showFilters)}
              className={`relative flex items-center gap-2 px-4 py-1.5 rounded-lg border text-[13px] font-semibold transition-colors ${showFilters ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-white border-gray-300 text-gray-700 hover:bg-gray-50"}`}
            >
            ...
            {(moduleFilter !== "Semua" || actionFilter !== "Semua" || search || searchInput) && (
              <button
                onClick={() => { setModuleFilter("Semua"); setActionFilter("Semua"); setSearch(""); setSearchInput(""); }}
                className="flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-semibold text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors whitespace-nowrap"
              >
                <X className="w-4 h-4" />
                Reset
              </button>
        ...
        {showFilters && (
          <div className="flex flex-wrap items-center gap-4 pt-3 border-t border-gray-100 animate-in slide-in-from-top-2 duration-200">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Modul:</span>
              <select
                value={moduleFilter}
                onChange={(e) => setModuleFilter(e.target.value)}
                className="bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-[13px] text-gray-700 outline-none focus:border-[#0A356A] cursor-pointer font-medium"
              >
            ...
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wide">Aksi:</span>
              <select
                value={actionFilter}
      ...
      <div className="bg-white border-x border-b border-gray-200 rounded-b-xl shadow-sm overflow-hidden">
        <table className="w-full table-fixed text-left border-collapse">
          <thead className="bg-gray-50/80 border-b border-gray-200">
            <tr>
              <th className="w-[12%] px-3 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Waktu</th>
              <th className="w-[14%] px-3 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Aktor</th>
              <th className="w-[15%] px-3 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Modul / Aksi</th>
              <th className="w-[17%] px-3 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Entitas</th>
              <th className="w-[27%] px-3 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider">Deskripsi</th>
              <th className="w-[15%] px-3 py-3 text-[11px] font-bold text-gray-500 uppercase tracking-wider text-center">Perubahan</th>
            </tr>
          </thead>
            ...
            {isLoading ? (
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center">
                  ...
                  <p className="text-[13px] font-medium text-gray-600">Memuat data audit trail...</p>
                </td>
              </tr>
              ...
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center">
                  ...
                  <p className="text-sm font-semibold text-gray-600">Tidak ada data ditemukan</p>
                  <p className="text-xs text-gray-400 mt-1">Coba sesuaikan filter pencarian.</p>
                </td>
              </tr>
            ) : (
              paginatedLogs.map((log) => {
                const config = actionTypeConfig[log.actionType] || actionTypeConfig.REVIEW;
                return (
                  <tr key={log.id} className="hover:bg-blue-50/20 transition-colors">
                    <td className="px-3 py-3.5 text-[12px] text-gray-600 font-medium leading-snug">
                      {formatDate(log.timestamp)}
                    </td>
                    <td className="px-3 py-3.5">
                      <p className="text-[13px] font-bold text-gray-900 truncate" title={log.actor}>{log.actor}</p>
                      <p className="text-[11px] text-gray-500 font-medium truncate" title={log.actorRole}>{log.actorRole}</p>
                    </td>
                    <td className="px-3 py-3.5">
                      <p className="text-[12px] text-gray-400 font-semibold truncate" title={log.module}>{log.module}</p>
                      <span className={`inline-flex mt-0.5 px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap ${config.badge}`}>
                        {log.action}
                      </span>
                    </td>
                    <td className="px-3 py-3.5">
                      <p className="text-[13px] font-bold text-[#0A356A] truncate" title={log.entityCode}>{log.entityCode}</p>
                      <p className="text-[11px] text-gray-500 font-medium truncate" title={log.entityName}>{log.entityName}</p>
                    </td>
                    <td className="px-3 py-3.5">
                      <p className="text-[12px] text-gray-600 font-medium leading-snug line-clamp-2" title={log.description}>
                        {log.description}
                      </p>
                    </td>
                    <td className="px-3 py-3.5 text-center">
                      {log.oldValue && log.newValue && log.oldValue !== log.newValue && log.oldValue !== "-" ? (
                        <div className="flex flex-wrap items-center justify-center gap-1 text-[10px] font-bold">
                          <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full whitespace-nowrap">{log.oldValue}</span>
                          <span className="text-gray-400">→</span>
                          <span className="bg-[#E0F2FE] text-[#0284C7] px-2 py-0.5 rounded-full whitespace-nowrap">{log.newValue}</span>
                        </div>
                      ) : log.newValue ? (
                        <span className="inline-flex items-center justify-center text-[10px] font-bold px-2 py-0.5 rounded-full bg-[#E0F2FE] text-[#0284C7] whitespace-nowrap">{log.newValue}</span>
                      ) : (
        ...
        {!isLoading && filteredLogs.length > 0 && (
          <div className="px-6 py-3 border-t border-gray-200 bg-white flex flex-col sm:flex-row justify-between items-center gap-3">
            <span className="text-[12px] font-medium text-gray-500">
              Menampilkan {(currentPage - 1) * ITEMS_PER_PAGE + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, filteredLogs.length)} dari {filteredLogs.length} data (10 baris/halaman)
            </span>
            {totalPages > 1 && (
