	...
	const requests: ReuseRequest[] = (Array.isArray(data) ? data : []).map(
		(item: ReuseRequestApi) => {
			...
			return {
				...
				requesting_unit:
					item.installation_location || item.requesting_project || "Unit Operasi",
				target_plant:
					item.requesting_plant || eq.plant?.name || eq.plant?.description || "-",
				start_date: startDateStr,
				end_date: "-",
				justification:
					item.justification || item.notes || "Kebutuhan operasional unit kerja",
