...
import { Search, Download, Printer, Eye, Plus, Info } from "lucide-react";
...
export default function InspeksiDashboard() {
  ...
  const fallbackData: Inspection[] = [
    {
      id: 1,
      equipment: { equipment_code: "C-102", name: "Centrifugal Pump C-102" },
      inspection_date: "2023-10-24T00:00:00Z",
      require_action: { name: "KHUSUS" },
    ...
    {
      id: 2,
      equipment: { equipment_code: "HE-205", name: "Heat Exchanger HE-205" },
      inspection_date: "2023-11-15T00:00:00Z",
      require_action: { name: "RUTIN" },
      inspector: "Anita Wijaya",
      status: "Mendatang",
      notes: "",
    },
    {
      id: 3,
      equipment: { equipment_code: "V-409", name: "Pressure Vessel V-409" },
      inspection_date: "2023-11-02T00:00:00Z",
      require_action: { name: "RUTIN" },
    ...
    {
      id: 4,
      equipment: { equipment_code: "T-552", name: "Steam Turbine T-552" },
      inspection_date: "2023-11-18T00:00:00Z",
      require_action: { name: "KHUSUS" },
      inspector: "Budi Santoso",
      status: "Mendatang",
      notes: "",
    ...
    {
      id: 5,
      equipment: { equipment_code: "K-901", name: "Compressor K-901" },
      inspection_date: "2023-10-29T00:00:00Z",
      require_action: { name: "RUTIN" },
      inspector: "Hadi Siswoyo",
      status: "Selesai",
      notes: "",
    },
    {
      id: 6,
      equipment: { equipment_code: "P-101", name: "Feed Pump P-101" },
      inspection_date: "2023-11-20T00:00:00Z",
      require_action: { name: "RUTIN" },
      inspector: "Rudi Hartono",
      status: "Mendatang",
      notes: "",
    },
    {
      id: 7,
      equipment: { equipment_code: "E-302", name: "Cooler E-302" },
      inspection_date: "2023-10-15T00:00:00Z",
      require_action: { name: "RUTIN" },
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(
          process.env.NEXT_PUBLIC_API_URL + "/api/inspections",
        );
        if (res.ok) {
  ...
  const handleExportCSV = () => {
    ...
    const csvRows = data.map((row) => {
      const dateStr = new Date(row.inspection_date).toLocaleDateString(
        "id-ID",
    ...
    link.setAttribute(
      "download",
      `Export_Inspeksi_${new Date().toISOString().split("T")[0]}.csv`,
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3 shrink-0 print:hidden">
        <div>
          <div className="flex items-center gap-1.5 text-[11px] text-gray-500 mb-0.5">
            ...
            <span className="font-semibold text-[#0A356A] tracking-normal">
              Daftar Inspeksi Teknik
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1 min-h-0 overflow-hidden">
        ...
        <div className="lg:col-span-9 flex flex-col gap-3 min-h-0">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 shrink-0 print:hidden">
            {[
              ...
              >
                <p className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider leading-none mb-1">
                  {kpi.label}
                </p>
                <p className="text-lg font-extrabold text-[#0F172A] leading-none tabular-nums">
                  {loading ? "..." : kpi.value}
                </p>
              </div>
            ))}
          ...
          <div className="bg-white rounded border border-gray-200 flex flex-col flex-1 min-h-0 overflow-hidden print:shadow-none print:border-none">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    filteredData.map((row, i) => (
                      <tr
                        key={i}
                        className="hover:bg-[#F2F3F4] transition-colors group"
                      >
                        <td className="px-4 py-2 whitespace-nowrap text-[12px] font-bold text-[#0A356A]">
                          {row.equipment.equipment_code}
                        </td>
                        <td
                          className="px-4 py-2 text-[12px] text-gray-800 font-medium truncate max-w-[150px]"
                        ...
                        <td className="px-4 py-2 whitespace-nowrap">
                          {row.status === "Terlambat" && (
                            <span className="border text-[#DC2626] text-[10px] font-semibold px-2 py-0.5 rounded-sm">
                              Terlambat
                            </span>
                          )}
                          {row.status === "Mendatang" && (
                            <span className="border text-[#0556B3] text-[10px] font-semibold px-2 py-0.5 rounded-sm">
                              Mendatang
                            </span>
                          )}
                          {row.status === "Selesai" && (
                            <span className="border text-[#059669] text-[10px] font-semibold px-2 py-0.5 rounded-sm">
                              Selesai
                            </span>
                          )}
                        </td>
                        <td className="px-4 py-2 whitespace-nowrap text-right">
                          {row.status !== "Selesai" ? (
                            <Link
                              href={`/inspeksi/inspeksi-berkala/formInspeksi?equipmentId=${(row.equipment as any)?.id || row.id}`}
                              className="inline-block bg-white text-[#0A356A] hover:bg-[#0556B3] hover:text-white border border-[#E6E8EA] px-2.5 py-1 rounded text-[10px] font-bold transition-all"
                            >
                              Mulai
                            </Link>
                          ) : (
                            <div className="flex items-center justify-end gap-1 text-gray-400 opacity-80 group-hover:opacity-100 transition-opacity">
                              <button
                                className="p-1 hover:text-[#0A356A] hover:bg-gray-100 rounded transition-colors"
                                title="Lihat"
                              >
            ...
            <div className="px-4 py-1.5 border-t border-gray-200 bg-gray-50 text-[10px] font-medium text-gray-500 flex justify-between items-center shrink-0">
              ...
              <div className="flex items-center gap-1">
                <button className="p-0.5 text-gray-400 hover:text-[#0A356A] disabled:opacity-50 transition-colors">
                  &lsaquo;
                </button>
                <button className="w-5 h-5 rounded bg-[#0A356A] text-white font-bold">
                  1
                </button>
                <button className="p-0.5 text-gray-400 hover:text-[#0A356A] transition-colors">
                  &rsaquo;
                </button>
              </div>
            </div>
        ...
        <div className="lg:col-span-3 flex flex-col gap-3 shrink-0">
          <div className="bg-white rounded border border-[#E6E8EA] p-4">
            ...
            <h3 className="text-[12px] font-bold text-[#0F172A] mb-1">
              Panduan Inspeksi
            </h3>
            <p className="text-[11px] text-[#64748B] leading-relaxed mb-3">
              Aset idle wajib diinspeksi minimal satu kali setiap 3 bulan untuk
              memastikan integritas mekanis.
            </p>
            <a
              href="#"
              className="text-[11px] font-semibold text-[#0556B3] hover:underline flex items-center gap-1"
            >
              Unduh SOP{" "}
