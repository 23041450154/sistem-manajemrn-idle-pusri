...
import { Plus, Trash2, CheckCircle2, XCircle, Loader2, Database, ShieldCheck, Pencil, Eye, Search, RefreshCw } from "lucide-react";
import { getRequireActions, createRequireAction, deleteRequireAction } from "@/action/api";
...
export default function RekomendasiTindakanPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const result = await getRequireActions();
      setData(result || []);
    } catch (error) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const res = await createRequireAction(itemName, itemDesc);
    setIsSubmitting(false);

    if (res.success) {
      setNotification({ type: "success", message: "Berhasil menambahkan tindakan inspeksi baru!" });
      setItemName("");
      setItemDesc("");
      setIsAddOpen(false);
      fetchData();
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    const res = await deleteRequireAction(selectedItem.id);
    setIsSubmitting(false);

    if (res.success) {
      setNotification({ type: "success", message: "Berhasil menghapus tindakan inspeksi!" });
  ...
  const handleEditSaved = (updatedItem: any) => {
    ...
    setNotification({ type: "success", message: "Berhasil memperbarui tindakan inspeksi!" });
    setTimeout(() => setNotification(null), 3000);
  };
    ...
    <div className="max-w-7xl mx-auto pt-6 pb-12 px-4 sm:px-6">
      ...
      <div className="bg-[#0A356A] rounded-2xl px-6 py-5 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white tracking-tight">
              Master Data — Rekomendasi Tindakan
            </h1>
            <p className="text-xs text-blue-200/90 mt-0.5 font-medium max-w-xl">
              Daftar opsi rekomendasi tindakan hasil inspeksi teknik untuk perbaikan / pemeliharaan.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>Tambah Tindakan Inspeksi</span>
        </button>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4 mb-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative w-full sm:w-[320px] shrink-0">
          ...
          <input
            type="text"
            placeholder="Cari rekomendasi tindakan..."
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden min-h-[420px]">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-xs font-semibold text-slate-500">Memuat data rekomendasi tindakan...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center px-4">
            ...
            <p className="text-xs text-slate-500 max-w-md">Tidak ada data rekomendasi tindakan yang cocok dengan pencarian Anda.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse table-fixed">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr className="h-[44px] align-middle">
                  ...
                  <th className="px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider text-left whitespace-nowrap align-middle w-[240px]">Nama Entri</th>
