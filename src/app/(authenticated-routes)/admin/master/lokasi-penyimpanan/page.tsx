...
import { Plus, Trash2, CheckCircle2, XCircle, Loader2, Database, MapPin, Pencil, Eye, Search, RefreshCw } from "lucide-react";
import { getStorageLocations, createStorageLocation, deleteStorageLocation } from "@/action/api";
...
export default function LokasiPenyimpananPage() {
  ...
  const filteredData = debouncedSearchQuery.trim()
    ? data.filter(item =>
        (item.name && item.name.toLowerCase().includes(debouncedSearchQuery.toLowerCase())) ||
        (item.plant && item.plant.toLowerCase().includes(debouncedSearchQuery.toLowerCase())) ||
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const res = await createStorageLocation(itemName, itemPlant, itemDesc);
    setIsSubmitting(false);

    if (res.success) {
      setNotification({ type: "success", message: "Berhasil menambahkan lokasi gudang baru!" });
      setItemName("");
      setItemDesc("");
      setIsAddOpen(false);
      fetchData();
  ...
  const handleDelete = async () => {
    ...
    setIsSubmitting(true);
    const res = await deleteStorageLocation(selectedItem.id);
    setIsSubmitting(false);

    if (res.success) {
      setNotification({ type: "success", message: "Berhasil menghapus lokasi gudang!" });
  ...
  const handleEditSaved = (updatedItem: any) => {
    ...
    setNotification({ type: "success", message: "Berhasil memperbarui lokasi gudang!" });
    setTimeout(() => setNotification(null), 3000);
  };
    ...
    <div className="max-w-7xl mx-auto pt-6 pb-12 px-4 sm:px-6">
      ...
      <div className="bg-[#0A356A] rounded-2xl px-6 py-5 mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-200/90 mt-0.5 font-medium max-w-xl">
              Daftar gudang dan lokasi fisik penyimpanan aset di setiap plant.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>Tambah Lokasi Gudang</span>
        </button>
      </div>

      {/* Filter & Search Bar (Enterprise 1-Row Responsive Layout) */}
      <div className="bg-white border border-slate-200 rounded-2xl px-5 py-4 mb-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative w-full sm:w-[320px] shrink-0">
          <Search className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" />
          <input
            type="text"
            placeholder="Cari lokasi gudang atau plant..."
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden min-h-[420px]">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="text-xs font-semibold text-slate-500">Memuat data lokasi gudang...</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center px-4">
            ...
            <p className="text-xs text-slate-500 max-w-md">Tidak ada data lokasi penyimpanan yang cocok dengan pencarian Anda.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse table-fixed">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr className="h-[44px] align-middle">
                  <th className="px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider text-center w-[80px] whitespace-nowrap align-middle">ID</th>
                  <th className="px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider text-left whitespace-nowrap align-middle w-[220px]">Nama Entri</th>
                  <th className="px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider text-center w-[140px] whitespace-nowrap align-middle">Pabrik (Plant)</th>
                  <th className="px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider text-left whitespace-nowrap align-middle">Deskripsi Keterangan</th>
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {paginatedData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/80 transition-colors h-[52px] align-middle">
                    <td className="px-4 py-3.5 whitespace-nowrap text-sm font-mono font-medium text-slate-500 text-center align-middle">#{item.id}</td>
                    <td className="px-4 py-3.5 whitespace-nowrap text-sm font-semibold text-slate-900 truncate align-middle">
                    ...
                    <td className="px-4 py-3.5 text-sm text-slate-600 font-medium truncate align-middle" title={item.description}>
