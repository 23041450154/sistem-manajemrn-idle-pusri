...
import { Plus, Trash2, CheckCircle2, XCircle, Loader2, Database, Tag, Pencil, Eye, Search, RefreshCw } from "lucide-react";
...
export default function KategoriAsetPage() {
  ...
  const filteredData = debouncedSearchQuery.trim()
    ? data.filter(item => item.name && item.name.toLowerCase().includes(debouncedSearchQuery.toLowerCase()))
    : data;
            ...
            <table className="w-full text-left border-collapse table-fixed">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr className="h-[44px] align-middle">
                  ...
                  <th className="px-4 py-3 text-xs font-semibold text-slate-600 uppercase tracking-wider text-left whitespace-nowrap align-middle">Nama Entri</th>
