...
Authoritative visual system for the authenticated role surfaces:
`rendal/`, `unit-kerja/`, `pemeliharaan/`, `inspeksi/dashboard`.
Every component in scope implements this. No one-off values.

The tokens below are already committed in `src/app/globals.css`
(`--radius: 4px`, `--color-state-*`, `--color-brand`). This file is the prose
contract for those tokens, not a second source of truth.
...
- Page header: breadcrumb, H1, one-line description, right-aligned action cluster. Bottom border `1px #E6E8EA`. No card wrapper. Never a gradient banner, never a decorative blur blob, never a pill badge above the title.
- Stage rail (repair pipeline): the repair flow is **sequential**, so it renders as an ordered left-to-right rail of stages, each with count, label, and a 2px top rule in its state hue. A donut/pie is banned for this data — it discards stage order, which is the information the user came for.
- Nav row (module shortcut): a `divide-y` list of full-width rows, each with title, one-line purpose, and a right chevron. Not a grid of equal icon-tile cards.
- Pagination: windowed. Render first page, last page, current ±1, with `…` gaps. Never `Array.from({length: totalPages})` — 400 rows would emit 40 buttons.
